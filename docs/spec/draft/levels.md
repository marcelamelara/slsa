...
environment. In this track, provenance describes how the hosted build platform
...
The primary purpose of the Runtime track is to enable [auditing] that a
...
> [!WARNING]
> The Runtime track currently only considers _hosted_ build platforms.
...
has Build L2 (or higher) SLSA provenance showing how the image was built.
...
-   MUST run builds using a build image that was built by a hosted build platform that
    meets Runtime L1 requirements.

-   SHOULD verify the build image's Build L2+ provenance for the selected build image,
    and distribute evidence of the verification to consumers (e.g., using a [VSA])

<dt>Build Platform Requirements<dd>

-   Automatically generate and distribute Build L2+ provenance for hosted build
...
The build runtime environment is integrity measured and authenticated
when deployed, attesting to the initial state of the runtime.

<dt>Intended for<dd>

Organizations wanting to ensure that a specific build is running
in the expected build runtime.

<dt>Software Producer Requirements<dd>

-   MUST run builds on a hosted build platform that meets Runtime L2
    requirements.
...
-   The boot process of each build runtime MUST be measured and
    attested. In addition, the initial state of the build runtime's storage volume
    MUST be integrity measured and attested. The boot and volume
    attestations MUST be distributed to allow for independent
    verification.
-   A build ID MUST be generated and cryptographically bound to the build
    environment via attestation.

<dt>Benefits<dd>

-   Provides evidence that a build runtime environment deployed by a hosted build
